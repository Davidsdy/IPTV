...
def prepare(rType="MAIN"):
    ...
    try:
        ...
        # Create User
        printc("Criando user xtreamcodes")
...
def mysql(rUsername, rPassword):
    ...
    if rCreate:
        shutil.copy("/etc/mysql/my.cnf", "/etc/mysql/my.cnf.old")
        rFile = open("/etc/mysql/my.cnf.old", "w")
        rFile.write(rMySQLCnf)
        rFile.close()
        os.system("service mysql restart > /dev/null")
    printc("Senha root do MYSQL:", col.WARNING)
    for i in range(5):
        rMySQLRoot = raw_input("  ")
        print " "
        if len(rMySQLRoot) > 0: rExtra = " -p%s" % rMySQLRoot
        else: rExtra = ""
        printc("Apagar existente e criar ? Y/N", col.WARNING)
        ...
        try:
            ...
            return True
        except: printc("Senha invalida, tente novamente", col.FAIL)
    return False
...
if __name__ == "__main__":
    printc("Xtream Codes Reborn BETA - instalacao", col.OKGREEN, 2)
    rType = raw_input("  Tipo de instalacao [MAIN, LB]: ")
    print " "
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            printc("Iniciar instalacao ? Y/N", col.WARNING)
            if raw_input("  ").upper() == "Y":
                ...
                start()
                printc("Instalacao completa!", col.OKGREEN, 2)
                printc("Admin UI: http://%s:25500" % getIP())
                if rType.upper() == "MAIN":
                    printc("Please store your MySQL password!")
                    printc(rPassword)
            else: printc("Instalacao cancelada", col.FAIL)
        else: printc("Invalid entries", col.FAIL)
    else: printc("Tipo de instalacao invalida", col.FAIL)
